...
class MyWidget(QWidget):
    ...
    def initUI(self):
        ...
        self.stack.addWidget(self.createHomePage())  # 在售
    ...
    def createSettingsPage(self):
        ...
        page = QWidget()
        layout = QVBoxLayout(page)
        set_user=QPushButton("修改账户")
        set_user.clicked.connect(show_person_info())
        layout.addWidget(QLabel("账户设置页面"))
        layout.addWidget(QPushButton("修改密码"))
        layout.addWidget(QPushButton("退出登录"))
        re_main=QPushButton("返回主页面")
        re_main.clicked.connect(lambda: self.stack.setCurrentIndex(0))
        return page
